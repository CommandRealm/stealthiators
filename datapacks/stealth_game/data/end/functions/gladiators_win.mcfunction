...
execute if score $winners calculate matches 2.. run tellraw @a [{"translate":"Gladiators win!","color":"red"},{"translate":"\nThe winners were: ","color":"#ba666a"},{"selector":"@a[tag=winner]"}]
execute if score $winners calculate matches ..1 run tellraw @a [{"translate":"Gladiators win!","color":"red"},{"translate":"\nThe winner was: ","color":"#ba666a"},{"selector":"@a[tag=winner]"}]
advancement grant @a[tag=winner,tag=playing] only minecraft:custom/win_on_classic
tag @a remove winner
