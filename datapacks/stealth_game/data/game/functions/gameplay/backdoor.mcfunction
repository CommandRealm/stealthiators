...
execute if score $tp_left band_number matches 0 if score $check_band band_number matches ..0 run tellraw @s [{"text":"Teleported to band ","color":"gold"},{"text":"#","color":"yelow","bold":true},{"score":{"objective":"band_number","name":"$correct_band"}}]
