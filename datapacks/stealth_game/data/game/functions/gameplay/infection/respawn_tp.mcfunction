...
execute if score $check_band band_number matches 1.. run effect give @s slow_falling 1 2 true
execute if score $check_band band_number matches 1.. run tp @s @e[type=area_effect_cloud,tag=final_hallway,sort=nearest,limit=1]
...
execute if score $check_band band_number matches ..0 unless score $tp_left band_number matches 1..10 at @s unless block ~ ~ ~ structure_void run tp @s ~ ~ ~-1
execute if score $check_band band_number matches ..0 if score $tp_left band_number matches 1..10 run function game:gameplay/infection/respawn_tp