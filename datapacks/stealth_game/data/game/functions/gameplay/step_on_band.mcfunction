...
execute if block ~ 125 ~ diamond_block as @e[type=area_effect_cloud,sort=nearest,limit=1,tag=mark_band] run tellraw @a[tag=playing] [{"text":"A runner has reached band ","color":"red"},{"text":"#","bold":true,"color":"dark_red"},{"score":{"objective":"band_number","name":"$number"},"color":"dark_red","bold":true}]
...
execute as @a[team=gladiator,scores={g_class=3,drop_trap=1..}] at @s run give @s snow{CustomModelData:1,display:{Name:'{"text":"Trap","color":"dark_red","italic":false}'},HideFlags:63}
