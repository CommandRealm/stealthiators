...
execute as @a[tag=playing,team=sneaker,scores={blind_cooldown=1..,alchemist_limit=1..}] run title @s actionbar [{"translate":"[","color":"gold","bold":true},{"score":{"objective":"display_cooldown","name":"@s"},"color":"#b1a370","bold":true},{"translate":"]","color":"gold","bold":true}]