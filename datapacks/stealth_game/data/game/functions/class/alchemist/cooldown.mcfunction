scoreboard players remove @a[tag=playing,team=sneaker,scores={blind_cooldown=1..}] blind_cooldown 1
...
execute as @a[tag=playing,team=sneaker,scores={blind_cooldown=1..}] run title @s actionbar [{"text":"[","color":"gold","bold":true},{"score":{"objective":"display_cooldown","name":"@s"},"color":"#b1a370","bold":true},{"text":"]","color":"gold","bold":true}]